...
import { Search, ExternalLink, XCircle, AlertTriangle, RefreshCw } from "lucide-react";
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Quản lý toàn bộ chứng chỉ và vai trò người dùng trong hệ thống.</p>
        </div>
        <button
          onClick={loadData}
          disabled={loading}
      ...
      <div className="flex border-b border-border">
        {(["certs", "users"] as const).map((t) => (
          ...
          >
            {t === "certs" ? "Tất cả chứng chỉ" : "Quản lý người dùng & vai trò"}
          </button>
        ))}
      </div>
      ...
      {roleTab === "certs" ? (
        <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
            ...
            <table className="w-full min-w-[700px]">
              <thead>
                <tr className="border-b border-border bg-muted/40">
                  {["Mã chứng chỉ", "Sinh viên", "Bằng cấp", "Ngày cấp", "Trạng thái", "Thao tác"].map((h) => (
                  ...
                  <tr key={c.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1.5">
                        <button className="p-1.5 rounded-lg hover:bg-[#EFF6FF] text-muted-foreground hover:text-[#1E3A8A] transition-colors" title="Xem trên blockchain">
          ...
          <div className="px-5 py-3 border-t border-border flex items-center justify-between">
            <p className="text-xs text-muted-foreground">{filtered.length} / {certs.length} chứng chỉ</p>
          </div>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[520px]">
              ...
              <tbody className="divide-y divide-border">
                ...
                {users.map((u) => (
                  <tr key={u.email || u.name} className="hover:bg-muted/30 transition-colors">
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        ...
                        "bg-[#EFF6FF] text-[#1E3A8A] border border-blue-200"
                      }`}>
                        {ROLE_LABEL_VI[u.role] ?? u.role}
                      </span>
                    </td>
                  </tr>
                ))}
      ...
      <Modal open={!!revokeModal} onClose={() => setRevokeModal(null)} title="Thu hồi chứng chỉ">
        <div className="space-y-4">
          <div className="flex items-start gap-3 p-3.5 rounded-xl bg-red-50 border border-red-200">
            <AlertTriangle size={18} className="text-red-600 shrink-0 mt-0.5" />
            <p className="text-sm text-red-800 leading-relaxed">
              <strong>Hành động này sẽ thu hồi vĩnh viễn chứng chỉ {revokeModal}.</strong> Việc thu hồi sẽ được ghi lại trên blockchain. Người sở hữu chứng chỉ sẽ được thông báo và chứng chỉ sẽ không còn vượt qua được bước xác minh.
            </p>
          </div>
          <div>
            <label className="text-sm font-medium text-foreground block mb-1.5">Lý do thu hồi</label>
            <textarea
              rows={3}
              value={revokeReason}
